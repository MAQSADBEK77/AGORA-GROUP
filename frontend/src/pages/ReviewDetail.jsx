...
import { ArrowLeft, Brain, CheckCircle, AlertTriangle, AlertCircle, XCircle, User } from 'lucide-react'
...
export default function ReviewDetail() {
  ...
  useEffect(() => {
    api.get(`/images/${id}`).then(r => {
      setImage(r.data)
      if (r.data.review) {
        setForm({ label: r.data.review.label, description: r.data.review.description || '' })
      }
  ...
  async function loadAiPrediction() {
    ...
    try {
      ...
      setAiPred(data)
      if (!form.label) setForm(f => ({ ...f, label: data.label }))
  ...
  async function submitReview(e) {
    e.preventDefault()
    if (!form.label) return toast.error('Label tanlang')
    setSubmitting(true)
    try {
      await api.post(`/review/${id}`, { label: form.label, description: form.description })
  ...
  const similarCases = aiPred?.similar_cases
    ? (typeof aiPred.similar_cases === 'string'
        ? JSON.parse(aiPred.similar_cases)
        : aiPred.similar_cases)
    : []
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="card">
          ...
          <img
            src={`/uploads/${image.filename.includes('/') ? image.filename : image.file_path.split('/').pop()}`}
            alt="mammogram"
            ...
            <div className="flex items-center justify-between mb-3">
              ...
              {!aiPred && (
                <button onClick={loadAiPrediction} disabled={aiLoading}
                  ...
                  {aiLoading
                    ? <><div className="animate-spin h-3 w-3 border-2 border-gray-500 border-t-transparent rounded-full" /> Tahlil qilinmoqda...</>
                    : 'AI dan so\'rang'}
                </button>
              )}
            </div>
            ...
            {aiPred ? (
              <div>
                {(() => {
                  ...
                  const Icon = s.icon
                  return (
                    <div className={`flex items-center gap-3 p-3 rounded-lg border ${s.bg} mb-3`}>
                      <Icon size={20} className={s.color} />
                      <div>
                        <p className={`font-bold ${s.color}`}>{aiPred.label}</p>
                        <p className="text-xs text-gray-500">
                          Ishonch: {Math.round(aiPred.confidence * 100)}%
                        </p>
                      </div>
                    </div>
                  )
                ...
                {similarCases.length > 0 && (
                  <div>
                    <p className="text-xs text-gray-500 font-medium mb-2">Eng o'xshash holatlar:</p>
                    <div className="space-y-1">
                      {similarCases.slice(0, 3).map((c, i) => (
                        <div key={i} className="flex items-center justify-between text-xs bg-gray-50 rounded px-3 py-2">
                          <span className="text-gray-600">Rasm #{c.image_id} {c.patient && `— ${c.patient}`}</span>
                ...
                {similarCases.length === 0 && (
                  <p className="text-xs text-orange-600 bg-orange-50 rounded p-2">
                    Hali labeled rasm yo'q. Birinchi diagnoz qo'ying — AI o'rganib boradi.
                  </p>
                )}
              </div>
            ) : (
              <p className="text-sm text-gray-400 text-center py-4">
                AI tahlilni ko'rish uchun tugmani bosing
              </p>
            )}
          </div>
          ...
          {canReview && (
            <div className="card">
              ...
              <form onSubmit={submitReview} className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Diagnoz *</label>
                      ...
                      return (
                        <button key={lbl} type="button"
                          ...
                          className={`flex items-center gap-2 p-3 rounded-lg border-2 text-sm font-medium transition-all ${
                            selected ? `${s.bg} border-current ${s.color}` : 'border-gray-200 text-gray-600 hover:border-gray-300'
                          }`}>
                ...
                <button type="submit" disabled={submitting || !form.label}
                  ...
                  {submitting ? 'Saqlanmoqda...' : image.review ? 'Yangilash' : 'Tasdiqlash va Saqlash'}
                </button>

                <p className="text-xs text-gray-400 text-center">
                  Saqlash bilan AI yangi labeled rasm asosida o'zini yangilaydi
                </p>
              </form>
            </div>
          )}

          {/* Ko'rish rejimi (hamshira) */}
          {!canReview && image.review && (
            <div className="card">
